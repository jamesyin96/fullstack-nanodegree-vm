...
address = raw_input("Please enter the address of the server you want to"
                    " access, \n If left blank the connection will be set "
...
try:
    ...
    url = (address +
           '/restaurants?location=Buenos+Aires+Argentina&mealType=Sushi')
    h = httplib2.Http()
    ...
    url = (address +
           '/restaurants?location=Prague+Czech+Republic&mealType=Crepes')
    h = httplib2.Http()
    resp, result = h.request(url, 'POST')
    if resp['status'] != '200':
        raise Exception('Received an unsuccessful status code of %s' %
...
try:
    print ("Attempting Test 4: Changing the name, image, and address of "
           "the first restaurant to Udacity...")
    result = all_result
    restID = result['restaurants'][0]['id']
    url = (address +
           "/restaurants/%s?name=Udacity&address=2465+Latham+Street+Mountain+View+CA&image=https://media.glassdoor.com/l/70/82/fc/e8/students-first.jpg" % restID)
    h = httplib2.Http()
